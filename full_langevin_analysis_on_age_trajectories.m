...
% load('full_cross_val_20d_v9.mat', 'all_best_bins')
% map_bins = all_best_bins;
%% run analysis
...
for i = 1:n_subs
    ...
    elseif length(unstable_eq) == 1 % here is case 3 or 4
        ...
        % left domain
        if abs(stable_eq(deep_idx).domain(1)) == Inf
            x_left = mod.potential_eff.dom(2);
            y_left = normU(x_left);
            [slope_left, width_left, depth_left] = calc_slope(x_eq, y_eq, x_left, y_left);
        else
            x_un = stable_eq(deep_idx).domain(1);
            y_un = normU(x_un);
            [slope_left, width_left, depth_left] = calc_slope(x_eq, y_eq, x_un, y_un);
        end

        % right domain
        if abs(stable_eq(deep_idx).domain(2)) == Inf
            x_right = mod.potential_eff.dom(end-1);
            y_right = normU(x_right);
            [slope_right, width_right, depth_right] = calc_slope(x_eq, y_eq, x_right, y_right);
        else
            x_un = stable_eq(deep_idx).domain(2);
            y_un = normU(x_un);
            [slope_right, width_right, depth_right] = calc_slope(x_eq, y_eq, x_un, y_un);
        end

        if (abs(stable_eq(deep_idx).domain(1)) ~= Inf) && (abs(stable_eq(deep_idx).domain(2)) ~= Inf)
        ...
        elseif (abs(stable_eq(deep_idx).domain(1)) ~= Inf)
